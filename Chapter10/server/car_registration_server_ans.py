...
class RequestHandler(socketserver.StreamRequestHandler):
    ...
    Call = dict(
        ...
        GET_LICENSES_STARTING_WITH=(
            lambda self, *args: self.get_licenses_starting_with(*args)),
        NEW_REGISTRATION=(
    ...
    def handle(self):
        protocolVersion = 1
        SizeStruct = struct.Struct("!IB") #Проверяем размер и номер протокола
        info = self.rfile.read(SizeStruct.size)
        size, version = SizeStruct.unpack(info)
        if (version > protocolVersion):
            raise ValueError("client is incompatible")
        else:
        ...
        self.wfile.write(SizeStruct.pack(len(data), protocolVersion))
    ...
    def get_licenses_starting_with(self, start):
        ...
        cars.sort()
        right = left = bisect.bisect_left(cars, start)
        while right < len(cars) and cars[right].startswith(start):
            right +=1
        return (True, cars[left:right])
