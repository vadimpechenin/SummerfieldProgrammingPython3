...
def retrieve_car_details(previous_license):
    ...
    if not ok:
        print(data[0])
        while True:
            #Пробуем найти ближайший номер или список и пусть выбирает
...
def handle_request(*items, wait_for_reply=True):
    SizeStruct = struct.Struct("!IB") #Передаем два числа
    #Добавка - версия протокола
    protocolVersion = 1
    data = pickle.dumps(items, 3)

    try:
        with SocketManager(tuple(Address)) as sock:
            sock.sendall(SizeStruct.pack(len(data), protocolVersion))
            sock.sendall(data)
            if not wait_for_reply:
                return

            info = sock.recv(SizeStruct.size)
